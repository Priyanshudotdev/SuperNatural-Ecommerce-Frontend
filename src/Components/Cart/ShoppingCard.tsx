...
const ShoppingCard: React.FC<productDetails> = ({
  ...
  const [numberOfItem, setNumberOfItem] = useState(qty ?? 1);
  return (
      ...
      <div className="flex flex-col w-full h-full gap-y-4  ">
        <div className="flex items-start justify-between w-full cursor-pointer">
          <div className="flex flex-col  ">
            <h1 className="text-lg font-semibold md:text-[1.25rem] opacity-[0.8] md:leading-[2rem] ">{productTitle || name}</h1>
            <p className="text-sm md:text-[1rem] opacity-70">{productSize ? `Size: ${productSize}` : ""}</p>
          </div>
        ...
        <div className="flex justify-between items-center sm:mt-6">
          <div className="flex items-center space-x-2">
            ...
            <span className="px-2 font-semibold tracking-[1px] ">{numberOfItem}</span>
            <button
